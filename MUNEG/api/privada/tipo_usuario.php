...
if (isset($_GET[ACTION])) {
    ...
    switch ($_GET[ACTION]) {
        ...
        case READ_ALL:
            ...
            break;
            case "readAllDeleted":
                if ($result[DATA_SET] = $tipo_usuario->readAllDeleted()) {
                    $result[STATUS] = SUCESS_RESPONSE;
                } elseif (Database::getException()) {
                    $result[EXCEPTION] = Database::getException();
                } else {
        ...
        case CREATE:
            $_POST = $tipo_usuario->validateSpace($_POST);
            $result[EXCEPTION] = $tipo_usuario->setNombre($_POST['tipo_usuario']) ? null : 'Nombre incorrecto';
            if ($tipo_usuario->createRow()) {
                $result[MESSAGE] = 'Registro creado correctamente';
                $result[DATA_SET] = $tipo_usuario->readAll();
        ...
        case UPDATE:
            $_POST = $tipo_usuario->validateSpace($_POST);
            $result[EXCEPTION] = $tipo_usuario->setNombre($_POST['tipo_usuario_update']) ? null : 'Nombre incorrecto';
            $result[EXCEPTION] = $tipo_usuario->setVisibilidad($_POST['visibilidad']) ? null : 'Visibilidad no encontrada';
            $result[EXCEPTION] = $tipo_usuario->setId($_POST['id_tipo_usuario']) ? null : 'Tipo Acabado incorrecto';

            if ($tipo_usuario->updateRow()) {
