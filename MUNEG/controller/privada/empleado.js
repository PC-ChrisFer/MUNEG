...
const ENDPOINT_TIPO_EMPLEADO =
  SERVER + "privada/empleado.php?action=readAll";
...
async function fillComboBoxTipoEmpleado() {
    ...
    //@ts-ignore
    APIResponse.dataset.map(element => { getElementById('tipo_empleado').innerHTML += `<option value="${element.id_tipo_empleado}" > ${element.nombre_tipo} </option>`})
    //@ts-ignore
    APIResponse.dataset.map(element => { getElementById('tipo_empleado_update').innerHTML += `<option value="${element.id_tipo_empleado}" > ${element.nombre_tipo} </option>`})
}
...
async function fillComboxEstadoEmpleado() {
    ...
    //Obtiene todos los valores y los ordena en un array, presentandolos en el select
    //@ts-ignore
    APIResponse.dataset.map(element => { getElementById('estado_empleado').innerHTML += `<option value="${element.id_estado_empleado}" > ${element.nombre_estado} </option>`})
    //@ts-ignore
    APIResponse.dataset.map(element => { getElementById('estado_empleado_update').innerHTML += `<option value="${element.id_estado_empleado}" > ${element.nombre_estado} </option>`})
}
        ...
        content += ` 
            <tr>
                ...
                <td class="d-flex justify-content-center">
                    ...
                    <a onclick="guardarDatosEmpleadoEliminar(${row.id_empleado})" class="btn" href="#" id="button_ver_mas" data-bs-toggle="modal"
...
// FUNCION PARA GUARDAR LOS DATOS DEL TIPO DE EMPLEADO
// @ts-ignore
window.guardarDatosEmpleadoEliminar = (id_empleado) => {
    datos_empleado.id = id_empleado;
    // @ts-ignore
    $("#eliminar").modal("show");
  };
