 ...
 function fillTableTipoPropiedad (dataset) {
  ...
  // Se recorre el conjunto de registros (dataset) fila por fila a través del objeto row.
    ...
    // Se crean y concatenan las filas de la tabla con los datos de cada registro.
             ...
             <a onclick="guardarDatosUpdate(${row.id_tipo_usuario},${row.visibilidad})" class="btn" id="button_ver_mas">
...
window.guardarDatosUpdate = (id_tipoPropiedad,visibilidad) => {
    ...
    //@ts-ignore
    getElementById("switchButton").value = datos_tipoUsuario.visibilidad ? "visible"
    : "invisible";
    //@ts-ignore
...
getElementById("insert_form")?.addEventListener("submit", async (event) => {
    ...
    await saveRow(API_GESTION_TIPO_USUARIO, API_CREATE, parameters, fillTableTipoPropiedad);
...
getElementById("update_form")?.addEventListener("submit", async (event) => {
    ...
    await saveRow(API_GESTION_TIPO_USUARIO, API_UPDATE, parameters, fillTableTipoPropiedad);
