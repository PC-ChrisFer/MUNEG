...
window.generarReporteCasasMunicipios = async () => {
  ...
  obtenerDatosParaReporteMunicipios.dataset
    tableContenido += `
      <tr>
      <td>${element.codigo}</td>
      <td>${element.municipio}</td>
  ...
  await generatePDF(generatedHTML, getElementById("casas_").value + "municipio" + ".pdf")
  window.open("../../api/reporte/" + "casas_" + municipio + ".pdf");
};